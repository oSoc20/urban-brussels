  ...
  searchFunction: () => {
    ...
    search.addEventListener('click', async (e) => {
      ...
      const send = {
        ...
        zipcode: '',
        cities: tagsSet.city,
        typologies: tagsSet.type,
        styles: tagsSet.style,
        intervenants: tagsSet.architect,
        streets: tagsSet.street
      }

      // TOT DELETE WHEN MULTIPLE VALUES ARE POSSIBLE
      if (tagsSet['zip code'].length !== 0) {
        send.zipcode = tagsSet['zip code'][0]
      }

      // if (tagsSet.city.length !== 0) {
      //   send.city = tagsSet.city[0]
      // }

      // if (tagsSet.type.length !== 0) {
      //   send.typology = tagsSet.type[0]
      // }

      // if (tagsSet.style.length !== 0) {
      //   send.style = tagsSet.style[0]
      // }

      // if (tagsSet.architect.length !== 0) {
      //   send.architects = tagsSet.architect[0]
      // }

      // if (tagsSet.street.length !== 0) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i].toUpperCase().includes(val.toUpperCase())) {
          ...
          b.addEventListener('click', function (e) {
            ...
            // Add tag
            if (!tagsSet[name.toLowerCase()].includes(value)) {
              ...
              // tagsSet.add(value.substr(0, value.indexOf('(')-1));
              tagsSet[name.toLowerCase()].push(value)
              ...
              for (let i = 0; i < tags.length; i++) {
                tags[i].addEventListener('click', () => {
                  ...
                  const index = tagsSet[name.toLowerCase()].indexOf(value)
                  if (index > -1) {
                    tagsSet[name.toLowerCase()].splice(index, 1)
                  }

                  // for (const o in tagsSet) {
                  //   if (tagsSet[o].length !== 0) {
                  //     noSearchItem = false
                  //   }
                  // }
